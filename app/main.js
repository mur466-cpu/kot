...
async function renderMainPage() {
  ...
  // Hero Slider - Only first trailer
...
function playMovie() {
  ...
  info.innerHTML = `<h3>${movie.title}</h3><p>Language: ${formatText(movie.language)} | Type: ${formatText(movie.type)}</p>`;
}

// Handle TV Remote Navigation
document.addEventListener("keydown", (e) => {
  const focusable = Array.from(document.querySelectorAll("a, button, .movie-card, .lang-card, .hero-play, input"));
